...
if(!isset($_SESSION['comprador'])){
  ...
  $sql = $pdo->query("SELECT pnome AS nome1, unome AS nome2 FROM usuarios WHERE id = '$_SESSION[comprador]'");
                    ...
                    <nav class="main-nav">
                        ...
                        <ul class="nav">
                            ...
                            <li class="scroll-to-section" title="Ver carrinho">
                                <a href="adicionarcarrinho.php"><i style="font-size: 2rem;"
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <div class="section-heading">
                            ...
                            <h3><a href="factura.php">Emitir Factura&nbsp;<i class="fa fa-list-alt"
                                        aria-hidden="true"></i></a></h3>
                            <p>*Se emitir a factura por duas ou mais vezes, não será possível visualizar a mesma a menos
                                que seja o primeiro documento baixado.</p>
                            <hr>
                        </div>
                    </div>
                </div>
...
$cliente = $nomes['nome1'].' '.$nomes['nome2']; 
$vendedor = $consulta['pnome'].' '.$consulta['unome'];
    foreach ($_SESSION['dados'] as $produtos) {
    $sql = $pdo->prepare("INSERT INTO vendas (id_produto, comprador, nome_comprador, vend, vendedor, qtd, preco_stock, preco_pedido, total, cod_fat) VALUES (?,?,?,?,?,?,?,?,?,?)");
    $sql->bindParam(1, $produtos['idProduto']);
    $sql->bindParam(2, $produtos['comprador']);
    $sql->bindParam(3, $cliente);
    $sql->bindParam(4, $consulta['id']);
    $sql->bindParam(5, $vendedor);
    $sql->bindParam(6, $produtos['qtd']);
    $sql->bindParam(7, $produtos['preco_stock']);
    $sql->bindParam(8, $produtos['preco_pedido']);
    $sql->bindParam(9, $produtos['total']);
